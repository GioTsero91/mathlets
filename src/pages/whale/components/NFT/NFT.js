...
const NFT = () => {
  ...
  const activities = Array.from({length: 50}, () => {
    return {
      image: NftImages[Math.floor(Math.random() * NftImages.length)],
      color: activityTypes[selectedActivity].color,
      name: "Bored Ape Yacht Club",
      price: "116.53",
        ...
        </div>
        {
          activityTypes.map((activity, key) => {
            return <div
              ...
              style={{opacity: selectedActivity === key ? "1" : "0.5"}}>
              <div className={"w-[14px] h-[14px] rounded-full"} style={{backgroundColor: activity.color}}></div>
              <div className={"text-[14px]"}>{activity.label}</div>
            </div>
          })
        }
      </div>
